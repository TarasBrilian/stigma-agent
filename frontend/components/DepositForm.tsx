 ...
 * set_price); the sole numeric work is ENCODING the typed dollar amount via
...
const STATUS_LABEL: Record<Exclude<Status, "idle" | "error">, string> = {
  ...
  pending: "Deposit submitted — the agent will invest it shortly.",
};
...
export function DepositForm({ vaultHash }: { vaultHash: string }) {
  ...
  const busy =
    status === "approving" || status === "confirming" || status === "depositing";
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      setStatus("pending");
      setAmount("");
      // Reconcile against backend reads once the deposit finalizes (~1 block).
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onKeyDown={(e) => e.key === "Enter" && !busy && void handleDeposit()}
              placeholder="100.00"
              disabled={busy}
